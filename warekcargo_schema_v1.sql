-- WarekCargo PostgreSQL Schema v1
-- Focus: customer registration, hub intake, package tracking, batch scheduling,
-- shipment consolidation, and package photo metadata.
...
create table if not exists inbound_packages (
    ...
    hub_id uuid not null references hubs(id),
    expected_batch_id uuid references shipping_batches(id) on delete set null,
...
create index if not exists idx_inbound_packages_expected_batch_id on inbound_packages(expected_batch_id);
...
insert into hubs (code, name, city)
values
    ('JKT', 'Hub Jakarta', 'Jakarta'),
    ('SUB', 'Hub Surabaya', 'Surabaya'),
    ('MKS', 'Hub Makassar', 'Makassar')
