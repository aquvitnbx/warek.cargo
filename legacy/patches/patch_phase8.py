...
wf['name'] = "My workflow 4 (Endpoint Nala V8)"
if 'id' in wf:
    del wf['id']
...
for n in wf['nodes']:
    ...
    if n['name'] == 'API Internal Tracking':
        n['parameters']['queryParameters']['parameters'][0]['value'] = "={{ $('Unwrap JSON AI').item.json.extracted_data && $('Unwrap JSON AI').item.json.extracted_data.packages && $('Unwrap JSON AI').item.json.extracted_data.packages.length > 0 ? $('Unwrap JSON AI').item.json.extracted_data.packages[0].tracking_number : '' }}"
...
with open('/Users/achmadhabib/Documents/Claude/Projects/WarekCargo/n8n_v5_clone.json', 'w') as f:
    json.dump(wf, f, indent=2)

print('n8n_v5_clone.json generated.')
