...
fallback_prefilter_node = {
  "parameters": {
    ...
    "sendBody": True,
    "specifyBody": "json",
    "jsonBody": "={\n  \"phone\": \"={{ $json['phone'] }}\",\n  \"message\": \"🤖 Halo, Nala menyortir pesan Anda sebagai spam/terlalu panjang. Mohon kirim format pendaftaran resi dengan lebih ringkas.\"\n}",
    "options": {}
  },
...
# Sanitasi JS (Tetap dipertahankan untuk membersihkan nomor telepon murni)
sanitize_node = {
  "parameters": {
    "jsCode": "return [{\n  json: {\n    ...$json,\n    phone: ($json.body.phone || '').toString().replace(/^08/, '628').replace(/\\D/g, '')\n  }\n}];"
  },
...
# AI Parser (HTTP Node to OpenAI structured outputs fallback)
openai_node = {
  "parameters": {
    "method": "POST",
  ...
  "id": "a3a3a3a3-openai",
  "name": "AI Parser (GPT-4o-Mini)"
}
new_nodes.append(openai_node)

# Set Node to unwrap OpenAI output
unwrap_ai = {
  "parameters": {
    "mode": "raw",
    "jsonOutput": "={{ Object.assign({}, $json.phone ? {phone: $json.phone, message: $json.message} : {}, JSON.parse($json.choices[0].message.content)) }}"
  },
...
# Guardrail: Validasi Output AI
ai_is_valid = {
  "parameters": {
    "conditions": {"options": {}, "conditions": [
        {"id": "cond-1", "leftValue": "={{$json.is_data_complete}}", "rightValue": "true", "operator": {"type": "string", "operation": "equals"}},
        {"id": "cond-2", "leftValue": "={{$json.intent}}", "rightValue": "INTAKE", "operator": {"type": "string", "operation": "equals"}}
    ], "combinator": "and"},
    "options": {}
  },
  "type": "n8n-nodes-base.if",
...
clarification_reply = {
  "parameters": {
    ...
    "sendBody": True,
    "specifyBody": "json",
    "jsonBody": "={\n  \"phone\": \"={{ $json['phone'] }}\",\n  \"message\": \"={{ $json['intent'] === 'OTHER' ? 'Nala tidak mengerti pesan ini. Mohon gunakan format pendaftaran resi.' : $json['clarification_question'] || 'Data belum lengkap. Mohon lengkapi ' + $json['missing_critical_fields'].join(', ') }}\"\n}",
    "options": {}
  },
  "type": "n8n-nodes-base.httpRequest",
  "typeVersion": 4.4,
  "position": [base_x + 1250, base_y],
  "id": "w6w6w6w6-clarification",
  "name": "Balas Clarification"
}
new_nodes.append(clarification_reply)

# API Intake Internal (App Layer Gate)
api_intake_node = {
  "parameters": {
    "method": "POST",
  ...
  "type": "n8n-nodes-base.httpRequest",
  "typeVersion": 4.4,
  "position": [base_x + 1250, base_y - 400],
...
api_fallback_guard = {
  ...
  "type": "n8n-nodes-base.if",
  "typeVersion": 2.3,
  "position": [base_x + 1500, base_y - 400],
...
api_error_reply = {
  ...
  "type": "n8n-nodes-base.httpRequest",
  "typeVersion": 4.4,
  "position": [base_x + 1800, base_y - 200],
  "id": "y8y8y8y8-apidown",
  "name": "Balas API Down/Timeout"
}
new_nodes.append(api_error_reply)

# Wablas Success/Business Error Reply
wablas_final = {
  "parameters": {
    "method": "POST",
    "url": "https://sby.wablas.com/api/send-message",
  ...
  "type": "n8n-nodes-base.httpRequest",
  "typeVersion": 4.4,
  "position": [base_x + 1800, base_y - 600],
  "id": "bbbbbb-2222",
...
new_conn = {
  ...
  "Sanitasi Phone": {"main": [[{"node": "AI Parser (GPT-4o-Mini)", "type": "main", "index": 0}]]},
  "AI Parser (GPT-4o-Mini)": {"main": [[{"node": "Unwrap AI JSON", "type": "main", "index": 0}]]},
  "Unwrap AI JSON": {"main": [[{"node": "Data Lengkap & Intake?", "type": "main", "index": 0}]]},
  "Data Lengkap & Intake?": {
    "main": [
...
"""
with open('update_phase1.sql', 'w') as f:
    f.write(sql)
