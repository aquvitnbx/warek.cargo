...
type IntakeRequestPayload = {
  phone?: string;
  owner_name?: string;
  hub_selection?: string;
  packages?: IntakePackagePayload[];
};

type IntakeResult = {
  tracking_number: string;
  status: 'DUPLICATE' | 'MISSING_INFO' | 'CREATED';
  message: string;
};
...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as IntakeRequestPayload;
    const { phone, owner_name, hub_selection, packages } = body;
    ...
    try {
      ...
      const hubRes = await client.query(
        'SELECT id, city FROM hubs WHERE LOWER(city) = LOWER($1) OR LOWER(name) = LOWER($1) LIMIT 1',
        [hub_selection.trim()]
      );
      ...
      const empRes = await client.query(
        'SELECT id, full_name FROM public.employees WHERE whatsapp_number = $1 AND is_active = true LIMIT 1',
        [sanitizedPhone]
      );
      ...
      if (!employeeId) {
        ...
        const defaultDest = packages[0]?.destination_city || 'Nabire';
        const custIdempotentStr = `CUST-WALKIN-${packages[0]?.tracking_number || sanitizedPhone}`;
      ...
      for (const pkg of packages) {
        ...
        await client.query(
          `INSERT INTO inbound_package_status_history (
             inbound_package_id, to_status_code, changed_source, change_notes, changed_by_employee_id, changed_by
           )
           VALUES ($1, 'AWAITING_HUB_RECEIPT', 'INTERNAL_AUTOMATION', 'Pendaftaran intake melalui automation internal', $2, $3)`,
    ...
    console.error('Internal intake endpoint error:', err);
    return NextResponse.json({ success: false, code: 'INTERNAL_ERROR', human_message: 'Malfungsi internal saat mengolah intake.' }, { status: 500 });
  }
}
