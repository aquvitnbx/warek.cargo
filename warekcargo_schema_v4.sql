-- WarekCargo PostgreSQL Schema v4
-- Canonical schema after Foundation Freeze.
-- VPS runtime: PostgreSQL + n8n + WhatsApp gateway + ERP Hub Next.js.
-- Nala-specific runtime/session objects are intentionally excluded.
...
create table if not exists customer_shipments (
    ...
    service_type_code text not null references ref_service_types(code),
    batch_container_id uuid references batch_containers(id) on delete set null,
    container_number text,
    vessel_name text,
    voyage_number text,
...
create table if not exists notifications_log (
    ...
    error_reason text,
    delivery_status_code text not null check (delivery_status_code in ('QUEUED', 'SENT', 'FAILED', 'SKIPPED', 'CANCELLED')),
    sent_at timestamptz,
    created_at timestamptz not null default now()
);
...
create index if not exists idx_employees_whatsapp_number on employees(whatsapp_number);
create index if not exists idx_batch_containers_batch_id on batch_containers(batch_id);
...
create index if not exists idx_customer_shipments_batch_container_id on customer_shipments(batch_container_id);
...
create trigger trg_batch_containers_set_updated_at before update on batch_containers for each row execute function set_updated_at();
